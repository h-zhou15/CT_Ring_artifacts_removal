function [ f_corr ,delta_f] = ring_artifact_removal( f_0,para,ring_center )
%RING_ARTIFACT_REMOVAL This function is used to remove the ring artifacts of reconstructed image. It is based
%                        on radial median filtering in cartesian coordinates(RCC).
% usuage:
...
% Reference:
   % Prell, D., et al. (2009). "Comparison of ring artifact correction methods for flat-detector CT." 
   %   Physics in Medicine and Biology 54(12): 3881.
...
for i = 1:Iwidth*Iheight
    if (f_0(i) < para.T_max)&&(f_0(i)>para.T_min)
        f_thres(i) = f_0(i);
        count = count +1;
    end
end
...
%             disp(sct);
            ...
            dxe = dx/dR;
            dye = dy/dR;
            for n = 1:2*para.M_rad+1
                xs(n) = round(x0+(dR - n*dRA)*dxe);
                ys(n) = round(y0+(dR - n*dRA)*dye);
                fs(n) = f_0(xs(n),ys(n));
            end
            sort_fs = sort(fs);
            mean_fs = sort_fs(para.M_rad+1); % get median value.
            f_M(x, y) = mean_fs;
